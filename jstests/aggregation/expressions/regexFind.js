...
(function() {
    ...
    /*testRegexFindAgg(9, {input:"$text", regex: "(F o o) # a comment\n\n(line) # another comment", options:"x"}, 
                    [{"_id" : 9, "matches" : 
                                [{"match" : "FooDEBUG", 
                                "idx" : 0, "captures": []}]
                    }]); */
    ...
    // Test: redaction 
    assert.writeOK(coll.insert({_id: 0, message:"Use my card 5500 0000 0000 0004", 
                                        user:"John Doe"}));
    assert.writeOK(coll.insert({_id: 1, message:"I'm 30 years old.", 
                                        user:"Jane Doe"}));

    let result = coll.aggregate([ {"$project":{ "message": {
                                   "$cond": [ {"$eq": [ 
                                                {"$regexFind": 
                                                    {input:"$message", 
