...
(function() {
    ...
    assert.writeOK(coll.insert({_id: 2, expanded: []}));
    result = coll.aggregate([{$match: {_id: 2}}, array_to_object_expr]).toArray();
    assert.eq(result, [{_id: 2, collapsed: {}}]);

    assert.writeOK(coll.insert({_id: 3}));
    result = coll.aggregate([{$match: {_id: 3}}, array_to_object_expr]).toArray();
    assert.eq(result, [{_id: 3, collapsed: null}]);
