...
(function (){
    ...
    assertErrorCode(coll, [ {$match: {_id: 2}}, array_to_object_expr ], 40384);

    assert.writeOK(coll.insert({_id: 2, expanded: [ ["item", "apple"], {"k": "price", "v": 24}] }));
    assertErrorCode(coll, [ {$match: {_id: 2}}, array_to_object_expr ], 40381);


    assert.writeOK(coll.insert({_id: 10, expanded: "string"}));
    assertErrorCode(coll, [ {$match: {_id: 10}}, array_to_object_expr ], 40379);

    assert.writeOK(coll.insert({_id: 11, expanded: ObjectId()}));
    assertErrorCode(coll, [ {$match: {_id: 12}}, array_to_object_expr ], 40379);

    assert.writeOK(coll.insert({_id: 13, expanded: NumberLong(0)}));
    assertErrorCode(coll, [ {$match: {_id: 13}}, array_to_object_expr ], 40379);
    ...
    assertErrorCode(coll, [ {$match: {_id: 16}}, array_to_object_expr ], 40379);
    ...
    assertErrorCode(coll, [ {$match: {_id: 21}}, array_to_object_expr ], 40380);

    assert.writeOK(coll.insert({_id: 22, expanded: [ ["missing_value"] ]}));
    assertErrorCode(coll, [ {$match: {_id: 22}}, array_to_object_expr ], 40382);
    
    assert.writeOK(coll.insert({_id: 23, expanded: [ [123, 12] ]}));
    assertErrorCode(coll, [ {$match: {_id: 23}}, array_to_object_expr ], 40383);

    assert.writeOK(coll.insert({_id: 24, expanded: [ ["key", "value", "offset"] ]}));
    assertErrorCode(coll, [ {$match: {_id: 24}}, array_to_object_expr ], 40382);

    // Document types.
    assert.writeOK(coll.insert({_id: 25, expanded: {y: []}}));
    assertErrorCode(coll, [ {$match: {_id: 25}}, array_to_object_expr ], 40379);

    assert.writeOK(coll.insert({_id: 26, expanded: [{y: "x"}] }));
    assertErrorCode(coll, [ {$match: {_id: 26}}, array_to_object_expr ], 40386);

    assert.writeOK(coll.insert({_id: 27, expanded: [{k: "missing"}] }));
    assertErrorCode(coll, [ {$match: {_id: 27}}, array_to_object_expr ], 40386);

    assert.writeOK(coll.insert({_id: 28, expanded: [{k: 23, v: "string"}] }));
    assertErrorCode(coll, [ {$match: {_id: 28}}, array_to_object_expr ], 40387);

    assert.writeOK(coll.insert({_id: 29, expanded: [{y: "ignored", k:"item", v:"pear"}] }));
    assertErrorCode(coll, [ {$match: {_id: 29}}, array_to_object_expr ], 40385);
